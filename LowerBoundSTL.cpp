...
int main() {
   ...
   {
       cin >> val;
       vector <int>::iterator low = lower_bound(vArray.begin(), vArray.end(), val);
       
       if (vArray[low - vArray.begin()] == val)
           cout << "Yes " << (low - vArray.begin() + 1) << endl;
       else
           cout << "No " << (low - vArray.begin() + 1) << endl;
    // cout <<  << endl;
   }
   return 0;
}

// You  given N integers in sorted order. Also, you are given Q queries. 
// In each query, you will be given an integer and you have to tell whether that 
// integer is present in the array. If so, you have to tell at which index it is present 
// and if it is not present, you have to tell the index at which the smallest integer 
// that is just greater than the given number is present.

// Input Format:-

// The first line of the input contains the number of integers N.
//  The next line contains N integers in sorted order. The next line contains Q, the number of queries. 
